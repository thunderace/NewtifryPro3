...
public final class LocaleConstants {
     ...
     * plug-in
     * condition. When
     * Locale broadcasts this {@code LocaleConstants}, it will be sent directly to the package and
     * class of the
     * plug-in's {@code Activity}. The {@code LocaleConstants} may contain a store-and-forward
     * {@link #EXTRA_BUNDLE}
     ...
     * LocaleConstants}. If a single APK
     * wishes to export multiple plug-ins, it MAY implement multiple Activity instances that
     * implement this
     * {@code LocaleConstants}, however there must only be a single {@link #ACTION_QUERY_CONDITION}
     * receiver. In this
     * scenario, it is the responsibility of the Activities to store enough data in {@link
     * #EXTRA_BUNDLE} to
     * allow this receiver to disambiguate which "plug-in" is being queried. To avoid user
     * confusion, it is
     ...
     * Locale broadcasts this {@code LocaleConstants}, it will be sent directly to the package and
     * class of the
     * plug-in's {@code Activity}. The {@code LocaleConstants} may contain a {@link #EXTRA_BUNDLE}
     * that was previously
     ...
     * wishes to export multiple plug-ins, it MAY implement multiple Activity instances that
     * implement this
     * {@code LocaleConstants}, however there must only be a single {@link #ACTION_FIRE_SETTING}
     * receiver. In this
     * scenario, it is the responsibility of the Activities to store enough data in {@link
     * #EXTRA_BUNDLE} to
     * allow this receiver to disambiguate which "plug-in" is being fired. To avoid user confusion,
     * it is
     * recommended that only a single plug-in be implemented per APK.
     *
     * @see LocaleConstants#EXTRA_BUNDLE
     ...
     * When Locale
     * broadcasts this {@code LocaleConstants}, it will be sent directly to the package and class
     * of
     * the plug-in's
     * {@code BroadcastReceiver}. The {@code LocaleConstants} will contain a {@link #EXTRA_BUNDLE}
     * that was previously
     * set by the {@code Activity} result of {@link #ACTION_EDIT_SETTING}.
     * <p>
     * There MUST be only one {@code BroadcastReceiver} per APK that implements this {@code
     * LocaleConstants}.
     *
     * @see LocaleConstants#EXTRA_BUNDLE
     */
     ...
     * Locale broadcasts this {@code LocaleConstants}, it will be sent directly to the package and
     * class of the
     * plug-in's {@code BroadcastReceiver}. The {@code LocaleConstants} will contain a {@link
     * #EXTRA_BUNDLE} that was
     ...
     * requesting that
     * Locale query it via {@link #ACTION_QUERY_CONDITION}. This merely serves as a hint to Locale
     * that a
     * condition wants to be queried. There is no guarantee as to when or if the plug-in will be
     * queried after
     * this {@code LocaleConstants} is broadcast. If Locale does not respond to the plug-in
     * condition after a
     * {@link #ACTION_REQUEST_QUERY} LocaleConstants is sent, the plug-in SHOULD shut itself down
     * and stop requesting
     * requeries. A lack of response from Locale indicates that Locale is not currently interested
     * in this
     ...
     * LocaleConstants}.
     * <p>
     * Plug-in conditions SHOULD NOT use this unless there is some sort of asynchronous event that
     * has
     * occurred, such as a broadcast {@code LocaleConstants} being received by the plug-in.
     * Plug-ins
     * SHOULD NOT
     ...
     * Parcelable}
     * is not a long-term storage format. Also, plug-ins MUST NOT store any serializable object
     * that
     * is not
     ...
     * 500
     * kilobytes (base-10), while Locale further limits plug-in Bundles to about 100 kilobytes
     * (base-10).
     * Although the maximum size is about 100 kilobytes, plug-ins SHOULD keep Bundles much smaller
     * for
     ...
     * best-effort
     * basis. Best-effort means that Locale may not persist known values forever (e.g. last known
     * values could
     ...
     * <p>
     * The purpose of an UNKNOWN result is to allow a plug-in condition more than 10 seconds to
     * process a
     * requery. A {@code BroadcastReceiver} must return within 10 seconds, otherwise it will be
     * killed by
